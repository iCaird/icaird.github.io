...
- An example of a *cluster algebra*, i.e. has distinguised collections of generators (clusters) and certain rules allowing you to move between them (mutation/exchange relations).

- The whole algebra can be generated starting with just one cluster.
...
[We now want to understand Schur representations of this algebra. But what is this algebra?]{.fragment}
...
Tubular algebras are certain tubular extensions of tame concealed algebras. They have global dimension two. They are of tame (non-domestic) representation type.

They are determined up to derived equivalence by their extension type which must be one of $(2,2,2,2;\lambda)$, $(3,3,3)$, $(2,3,6)$, $(2,4,4).$
:::
...
Let $P$ be the tubular algebra obtained previously.

**Question**: Which tubes have $P$-modules of ambient dimension $r$ high up enough in them?

**Tactic**: Understand slope 1 and then use Ringel's shrinking functors to bounce around tubes and
track how the data changes.

In practice, we can do this numerically to see if there might be some structure and then work out a formula later.
:::
